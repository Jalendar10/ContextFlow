# AI / ML Engineer Interview Skill

## Purpose
Connect a modeling or AI-system decision to a measurable task, dependable evaluation and production constraints.
...
Use Task → Baseline → Data → Approach → Evaluation → Deployment → Monitoring → Outcome. For an LLM incident, identify the failure layer before proposing model changes.

### Task and baseline
Define the user decision, target behavior, available data and meaningful baseline. Check whether rules, search or conventional software are sufficient before introducing a model.

### Data and evaluation
Explain split strategy, leakage prevention, label quality and evaluation metrics appropriate to the task. Separate training results from holdout and production evidence. Include slice analysis and human review when relevant.

### LLM and agent systems
Describe retrieval and grounding, prompt/input construction, structured-output checks, tool boundaries and failure handling. Use agents only when dynamic decisions justify them. Explain evaluation cases, regression testing and prompt/model versioning.

### Production lifecycle
Cover latency, throughput, cost, observability, deployment, rollback and drift. Distinguish a model artifact from the complete serving system. State only supported performance gains and production responsibilities.

## Practice prompts
These are examples for preparation, not predictions of the actual interview.
- Why use an LLM for this problem?
- How would you evaluate a RAG application?
- What would make you reject a model with higher accuracy?
