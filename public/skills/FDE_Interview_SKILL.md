# Forward Deployed Engineer Interview Skill

## Purpose
Turn an ambiguous customer problem into a useful, adopted production solution.
...
For a customer scenario, structure the response as Problem → User → Uncertainty → Success criteria → Approach → Candidate contribution → Validation → Production → Result. Use only the steps needed for the question.

### Client discovery
Identify the actual user, current workflow, business pain, constraints and decision makers. Clarify what success means before selecting a tool. Separate the requested feature from the underlying problem.

### End-to-end delivery
Explain discovery → smallest useful experiment → user validation → integration → production rollout → adoption. Cover the candidate’s actual contribution, stakeholder feedback and what changed after testing.

### Engineering judgment
Discuss API and data integration, deployment, access control, latency, reliability and operating cost when relevant. Choose deterministic software when a fixed workflow is enough; justify AI or agent behavior only when the task needs it.

### Production and value
Distinguish a successful demonstration from a maintained service. Explain testing, observability, ownership, rollback and user adoption. Only name outcome metrics actually supplied in the references.

## Practice prompts
These are examples for preparation, not predictions of the actual interview.
- Why FDE rather than product engineering?
- Describe a vague client requirement you clarified.
- How did you take a prototype into production?
- What did you change after customer feedback?
