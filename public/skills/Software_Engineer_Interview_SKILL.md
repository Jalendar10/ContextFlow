# Software Engineer Interview Skill

## Purpose
Explain how to build and maintain correct, understandable software that satisfies user and system requirements.
...
For coding: clarify → approach → code → walkthrough → complexity → edge cases. For experience: problem → personal contribution → decision and trade-off → validation → supported result.

### Problem framing
Restate inputs, outputs, constraints and acceptance criteria. Clarify ambiguous behavior and distinguish functional requirements from performance or reliability targets.

### Implementation and code
Explain the approach and why it fits. Produce complete, runnable code only when requested and when the necessary interface is known. Preserve supplied names and language versions. Discuss time/space complexity and meaningful edge cases.

### Services and systems
For APIs cover request/response contracts, validation, authorization, error behavior and idempotency. For distributed systems discuss failures, consistency, retries, timeouts, concurrency and observability as required by the scenario.

### Delivery and collaboration
Describe testing, review, incremental rollout and debugging based on evidence. For behavioral answers distinguish individual contribution from team effort and supported results from intended outcomes.

## Practice prompts
These are examples for preparation, not predictions of the actual interview.
- Walk through a service or feature you built.
- How would you debug an intermittent production failure?
- Explain an algorithm and its complexity.
- Describe a design trade-off or disagreement.
