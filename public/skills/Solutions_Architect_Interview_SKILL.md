# Solutions Architect Interview Skill

## Purpose
Translate business and technical constraints into an implementable architecture with explicit trade-offs and ownership.
...
Use Goals → Constraints → Options → Recommended design → Trade-offs → Failure handling → Delivery plan → Validation. End with a small concrete example only when it clarifies the design.

### Discovery and constraints
Identify stakeholders, workflows, integration boundaries, existing systems, budget and delivery constraints. Separate stated requirements from assumptions needing confirmation.

### Architecture design
Explain components, data/control flow, interfaces, trust boundaries and deployment responsibilities. Start with the simplest design meeting the requirements. Compare realistic alternatives rather than listing products.

### Quality attributes
Address availability, failure isolation, recovery, security, scalability, operability and cost in proportion to the scenario. Do not invent numerical targets; ask for them or label proposed targets as hypothetical.

### Migration and adoption
Explain phased delivery, compatibility, validation, cutover, rollback and handover. Identify risks and how to reduce them. Connect recommendations to actual business needs and documented candidate experience.

## Practice prompts
These are examples for preparation, not predictions of the actual interview.
- Design an integration across legacy and cloud systems.
- How would you migrate with minimal disruption?
- How do you balance availability, complexity and cost?
- How would you explain a technical trade-off to a stakeholder?
