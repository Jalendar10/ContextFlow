...
export class ProviderAI {
 ...
 async generateOnce({question,evidence,history=[],signal,selection,system=SYSTEM,maxTokens=2200,onDelta}){
  ...
  else if(provider==='openai'){data=await this.remote(provider,'/responses',{...options,body:JSON.stringify({model,store:false,instructions:system,input,max_output_tokens:maxTokens,...(onDelta?{stream:true}:{})})});answer=data.output?.flatMap(x=>x.content||[]).filter(c=>c.type==='output_text').map(c=>c.text).join('\n');tokens=data.usage?.output_tokens;}
