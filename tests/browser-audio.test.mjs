...
test('tab capture resumes audio during user gesture and requests no focus transfer',async()=>{
 ...
 try{
  Object.defineProperty(globalThis,'navigator',{configurable:true,value:{mediaDevices:{getDisplayMedia:async options=>{calls.push('picker');assert.equal(options.controller.behavior,'no-focus-change');assert.equal(options.audio,true);assert.equal(options.systemAudio,options.video.displaySurface==='monitor'?'include':'exclude');return {getVideoTracks:()=>[track],getAudioTracks:()=>[track],getTracks:()=>[track]}}}}});
  ...
  const audio=new BrowserAudio();await audio.choose('tab',false);assert.deepEqual(calls,['resume','picker']);audio.stop();const system=new BrowserAudio();await system.choose('system',false);assert.ok(system.meeting);system.stop();await assert.rejects(audio.start('session',()=>{}),/ended before/);
