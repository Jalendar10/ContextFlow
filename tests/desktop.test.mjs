...
test('desktop captures bind process and bundle, and refreshed text becomes new evidence',async()=>{
 ...
 const desktop=new Desktop({platform:'darwin',store,binary:process.execPath,run:async(file,args)=>{calls.push(args);return {stdout:JSON.stringify({title:'Test app',text,pid:123,bundleId:app.bundleId,warnings:[]})}}});
...
test('live audio uses pinned provider, stores real transcript output, cleans chunks and stops',async()=>{
 ...
 const desktop=new Desktop({platform:'darwin',store,ai,binary:process.execPath,run:async()=>({stdout:JSON.stringify({available:true,apps:[app]})}),launch:()=>child});
 try{
 ...
 }finally{desktop.dispose();if(desktop.session?.cleanup)await desktop.session.cleanup;else if(desktop.session)await rm(desktop.session.directory,{recursive:true,force:true,maxRetries:5,retryDelay:100})}
});
